...
Requires Python 3.12+ and a CUDA-capable GPU. The pip package bundles the CUDA runtime libraries, so a separate CUDA toolkit installation is not required. CuPy or another DLPack-compatible GPU array library is needed to create the input arrays.
...
- For Python bindings: Python 3.12+, array-api-compat
- For MATLAB bindings: MATLAB R2018b+, Parallel Computing Toolbox

nanobind is downloaded automatically during the CMake configure step via FetchContent.
...
This installs ffdas in editable mode. The compiled extension must already be present from the CMake build step.

CuPy is not a hard dependency, but in practice you will need it (or another DLPack-compatible GPU array library) to create the GPU arrays that ffdas operates on.
