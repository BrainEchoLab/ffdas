"""Interpolation from structured grids to arbitrary positions.

A structured grid has a regular index-space lattice (nz, ny, nx) but
its vertex positions can be arbitrary 3D coordinates. This enables
reconstruction on curvilinear grids (e.g. spherical) followed by
interpolation to Cartesian coordinates or any other query positions.

This example places scatterers on a regular Cartesian grid, then
reconstructs on a spherical grid where the pattern appears warped.
After interpolation back to Cartesian coordinates, the grid is
recovered.
"""

import math

import cupy as cp
import numpy as np
import matplotlib.pyplot as plt

import ffdas


cp.random.seed(0)


# spherical reconstruction grid
nr, ntheta, nphi = 96, 64, 64
r = cp.linspace(0.005, 0.035, nr, dtype=cp.float32)
theta = cp.linspace(-0.4, 0.4, ntheta, dtype=cp.float32)
phi = cp.linspace(-0.4, 0.4, nphi, dtype=cp.float32)

rr, tt, pp = cp.meshgrid(r, theta, phi, indexing="ij")  # type: ignore
grid_points = cp.stack([
    rr * cp.sin(tt),
    rr * cp.sin(pp),
    rr * cp.cos(tt) * cp.cos(pp),
], axis=-1)


# simulate and reconstruct (see reconstruct.py for details)
pitch = 150e-6
el = (cp.arange(32, dtype=cp.float32) - 15.5) * pitch
ex, ey = cp.meshgrid(el, el, indexing="ij")  # type: ignore
channel_pos = cp.stack(
    [ex.ravel(), ey.ravel(), cp.zeros(1024, dtype=cp.float32)], axis=-1
)

sound_speed = 1540.0
center_freq = 5e6
sampling_freq = center_freq
n_samples = 512
batch_size = 32

# scatterers on a regular Cartesian grid. the x-range is kept within
# the spherical grid's angular extent (|theta| < 0.4 at z = 10 mm
# requires |x| < z*tan(0.4) = 4.2 mm)
xs = cp.linspace(-0.004, 0.004, 9, dtype=cp.float32)
ys = cp.linspace(-0.003, 0.003, 3, dtype=cp.float32)
zs = cp.linspace(0.010, 0.026, 9, dtype=cp.float32)
gzs, gys, gxs = cp.meshgrid(zs, ys, xs, indexing="ij")  # type: ignore
scatterers = cp.stack(
    [gxs.ravel(), gys.ravel(), gzs.ravel()], axis=-1
).astype(cp.float32)
n_scatterers = scatterers.shape[0]  # 243

source = cp.array([[0.0, 0.0, -0.005]], dtype=cp.float32)

freqs = cp.fft.fftfreq(n_samples, d=1.0 / sampling_freq) + center_freq
wavenums = (-2 * math.pi * freqs / sound_speed).astype(cp.float32)
sigma_f = 0.6 * center_freq / (2 * math.sqrt(2 * math.log(2)))
pulse = cp.exp(-0.5 * ((freqs - center_freq) / sigma_f) ** 2).astype(cp.complex64)

phases = cp.exp(
    2j * cp.pi * cp.random.rand(batch_size, n_scatterers, 1)
).astype(cp.complex64)

tx = ffdas.greens(pulse[None, None, :], source, scatterers, wavenums)
rx = ffdas.greens(tx * phases, scatterers, channel_pos, wavenums)
rf = cp.fft.ifft(rx, axis=-1).astype(cp.complex64).conj()
rf = rf[:, :, None, :]

k = sampling_freq / sound_speed
offsets = (ffdas.cdist(source[0], grid_points) * k)[None]
weights = cp.ones_like(offsets)

volume = ffdas.das(
    rf,
    channel_pos * k,
    grid_points * k,
    offsets,
    weights,
    wavenum=-2 * math.pi * center_freq / sampling_freq,
)

envelope = cp.abs(volume)  # (batch, nr, ntheta, nphi)


# the Interpolator builds a lookup structure over the grid vertices
# and can be reused for multiple query point sets
interp = ffdas.Interpolator(grid_points)

cart_nz, cart_ny, cart_nx = 128, 64, 64
cx = cp.linspace(-0.010, 0.010, cart_nx, dtype=cp.float32)
cy = cp.linspace(-0.010, 0.010, cart_ny, dtype=cp.float32)
cz = cp.linspace(0.005, 0.035, cart_nz, dtype=cp.float32)
czz, cyy, cxx = cp.meshgrid(cz, cy, cx, indexing="ij")  # type: ignore
cart_points = cp.stack([cxx, cyy, czz], axis=-1)

with ffdas.utils.Timer() as t:
    cart_volume = interp(envelope, cart_points, fill=0.0)
print(f"interpolate to {cart_nz}x{cart_ny}x{cart_nx}: {t.elapsed_ms():.1f} ms")


# the spherical volume in index space: a max projection over phi
# shows the Cartesian grid warped by the curvilinear coordinates.
# after interpolation to Cartesian, the grid is recovered
sph_mip = cp.asnumpy(envelope[0].max(axis=2))  # (nr, ntheta)
db_sph = 20 * np.log10(sph_mip / sph_mip.max() + 1e-10)

cart_mip = cp.asnumpy(cart_volume[0].max(axis=1))  # (cart_nz, cart_nx)
db_cart = 20 * np.log10(cart_mip / cart_mip.max() + 1e-10)

fig, axes = plt.subplots(1, 2, figsize=(10, 6))

axes[0].imshow(
    db_sph,
    extent=[float(theta[0]), float(theta[-1]),
            float(r[-1]) * 1e3, float(r[0]) * 1e3],
    cmap="gray",
    vmin=-40,
    vmax=0,
    aspect="auto",
)
axes[0].set_xlabel("θ [rad]")
axes[0].set_ylabel("r [mm]")
axes[0].set_title("spherical grid (warped)")

axes[1].imshow(
    db_cart,
    extent=[float(cx[0]) * 1e3, float(cx[-1]) * 1e3,
            float(cz[-1]) * 1e3, float(cz[0]) * 1e3],
    cmap="gray",
    vmin=-40,
    vmax=0,
    aspect="equal",
)
axes[1].set_xlabel("x [mm]")
axes[1].set_ylabel("z [mm]")
axes[1].set_title("Cartesian (interpolated)")

plt.tight_layout()
plt.savefig("interpolation.png", dpi=150)
