...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs != 1)
        mexErrMsgIdAndTxt("ffdas_event_elapsed_time:nargs", "expected 1 argument: ffdas_event_elapsed_time(event)");
    if (nlhs != 0)
    ...
    if (!mxIsUint64(mx) || mxIsEmpty(mx))
        mexErrMsgIdAndTxt("ffdas:event_synchronize", "invalid event");
