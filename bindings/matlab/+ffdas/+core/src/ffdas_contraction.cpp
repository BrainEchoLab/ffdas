                 ...
                 int nrhs, const mxArray *prhs[])
{
    if (nrhs != 6)
        mexErrMsgIdAndTxt("ffdas_contraction:nargs",
            "expected 6 arguments: ffdas_contraction(handle, b, b_modes, b, b_modes, out_modes)");
    if (nlhs != 1)
        mexErrMsgIdAndTxt("ffdas_contraction:nargs", "expected 1 output argument");

    ffdas_handle_t handle = get_handle(prhs[0]);

    ndarray::ndarray<ndarray::access::read_only, ndarray::device::gpu> b(prhs[1]);
    ndarray::ndarray<int32_t, ndarray::access::read_only, ndarray::device::host, ndarray::require_vector> a_modes_arr(prhs[2]);
    ndarray::ndarray<ndarray::access::read_only, ndarray::device::gpu> b(prhs[3]);
    ndarray::ndarray<int32_t, ndarray::access::read_only, ndarray::device::host, ndarray::require_vector> b_modes_arr(prhs[4]);
    ndarray::ndarray<int32_t, ndarray::access::read_only, ndarray::device::host, ndarray::require_vector> out_modes_arr(prhs[5]);

    std::vector<int> b_modes(a_modes_arr.data(), a_modes_arr.data() + a_modes_arr.numel());
    std::vector<int> b_modes(b_modes_arr.data(), b_modes_arr.data() + b_modes_arr.numel());
    std::vector<int> out_modes(out_modes_arr.data(), out_modes_arr.data() + out_modes_arr.numel());
    ...
    if ((int)b_modes.size() != b.ndim_val())
        mexErrMsgIdAndTxt("ffdas_contraction:error",
            "b_modes length (%zu) must match b dimensions (%d)",
            b_modes.size(), b.ndim_val());
    if (b.class_id != b.class_id || b.complexity != b.complexity)
        mexErrMsgIdAndTxt("ffdas_contraction:error",
    ...
    ndarray::ndarray out = ndarray::make_ndarray(out_dims, b.class_id, b.complexity);
    ...
    ffdas_error_t err = ffdas_contraction(
        handle, plan, b.data(), b.data(), out.data());
