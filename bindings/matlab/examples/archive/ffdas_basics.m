...
[params, data] = load_sample_data(data_dir, 4);  % load 4 chunks
...
data = ffdas.gather(data, params.channel_map+1, 4, [1, 2, 4, 3, 5], 'single');
...
wavenum = -2 * pi * params.demod_frequency / params.sampling_frequency;

xdir = repmat(cast(gpuArray([0.0, 0.0, 1.0, 0.707]'), 'single'), [1, size(data, 3)]);

for i=1:10
    fprintf('beamforming %d frames...', size(data, 4));
    tic;
    beamformed = ffdas.das(...
        data, ...
...
disp(size(beamformed));
disp(beamformed(1:8) * 1e-5);
...
output_file = 'ffdas_basics_output.png';
