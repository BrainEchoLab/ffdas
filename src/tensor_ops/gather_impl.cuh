...
ffdas_error_t gather_impl(
    ...
    // Decompose the output's linear index into nd coordinates on the GPU
    ...
    CUDA_CHECK(cudaMemcpyToSymbolAsync(gather_strides, h_strides.data(), ndim * sizeof(int), 0, cudaMemcpyHostToDevice, handle.stream));
    CUDA_CHECK(cudaMemcpyToSymbolAsync(gather_radix, radix.data(), ndim * sizeof(unsigned), 0, cudaMemcpyHostToDevice, handle.stream));
